...
export default {
  ...
  methods: {
    handleChange(row) {
      request.put("/api/role/save", row).then(res => {
    ...
    load() {
      ...
      request.get("/api/permission/getAll").then(res => {
        this.permissions = res.pinfo
      })
    },
    add() {
    ...
    save() {
      if (this.form.id) {  // 更新
        request.put("/api/role/update", this.form).then(res => {
          console.log(res)
          if (res.code === 1) {
            this.$message({
              type: "success",
        ...
        request.post("/api/role/create", this.form).then(res => {
          if (res.code === 1) {
            this.$message({
              type: "success",
              message: "新增成功"
    ...
    handleDelete(id) {
      request.post("/api/role/delete", id).then(res => {
        if (res.code === 1) {
          this.$message({
            type: "success",
            message: "删除成功"
